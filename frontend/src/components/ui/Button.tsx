...
const SIZES: Record<Size, { h: number; px: number; font: number; icon: number; gap: number }> = {
  sm: { h: 36, px: 12, font: 13, icon: 14, gap: 6 },
  md: { h: 46, px: 18, font: 15, icon: 16, gap: 8 },
  lg: { h: 54, px: 22, font: 16, icon: 18, gap: 10 },
};

const VARIANTS: Record<Variant, { bg: string; bgPressed: string; fg: string; border?: string }> = {
  primary:     { bg: COLORS.primary,      bgPressed: "#1A4EC4",           fg: "#FFFFFF" },
  secondary:   { bg: COLORS.surface,      bgPressed: COLORS.primaryLight, fg: COLORS.text, border: COLORS.border },
  destructive: { bg: COLORS.danger,       bgPressed: COLORS.dangerDark,   fg: "#FFFFFF" },
  ghost:       { bg: "transparent",       bgPressed: COLORS.primaryLight, fg: COLORS.primary },
};

export function Button({
  title, onPress, variant = "primary", size = "md",
  icon, iconRight, loading = false, disabled = false,
  fullWidth = false, style, testID,
}: ButtonProps) {
  ...
  const containerStyle = (state: PressableStateCallbackType): ViewStyle => ({
    ...
    backgroundColor: state.pressed || hovered ? v.bgPressed : v.bg,
...
export function IconButton({
  icon, onPress, variant = "secondary", size = "md",
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [{
        width: sz.h, height: sz.h, borderRadius: RADIUS.sm,
